...
DWORD WINAPI CIOCPModel::_WorkerThread(LPVOID lpParam)
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		if (!bReturn)
		{
			...
			{
				std::cout << "worker exit!!!!!!!!!!!!!!______warning\n";
				break;
			}
			std::cout << "whiling!!!\n";
			continue;
		...
		else
		{
			...
			{
				std::cout<<"客户端 "<<inet_ntoa(pSocketContext->m_ClientAddr.sin_addr)<< ntohs(pSocketContext->m_ClientAddr.sin_port) <<" 断开连接.\n";
			...
			else
			{
				...
				case ACCEPT_POSTED:
				{
					...
					if (FALSE == pIOCPModel->_DoAccpet(pSocketContext, pIoContext))
						printf("warning : doaccept faild!!!\n");
...
bool CIOCPModel::Start()
{
	...
	{
		std::cout<<"Listen Socket初始化失败！\n";
		this->_DeInitialize();
		return false;
	}
	else
	{
		std::cout<<"Listen Socket初始化完毕.";
	}

	std::cout<<"系统准备就绪，等候连接....\n";
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{
		std::cout << "初始化Socket失败，错误代码: " << WSAGetLastError() << "\n";
		return false;
	}
	else
	{
	...
	ServerAddress.sin_addr.s_addr = inet_addr(m_strIP.data());
	ServerAddress.sin_port = htons(m_nPort);

	// 绑定地址和端口
	if (SOCKET_ERROR == bind(m_pListenContext->m_Socket, (struct sockaddr *) &ServerAddress, sizeof(ServerAddress)))
	{
		std::cout<<"bind()函数执行错误.\n";
		return false;
	}
	else
	{
	...
	{
		std::cout << "绑定 Listen Socket至完成端口失败！错误代码:" << WSAGetLastError() << "/n";
	...
	if (SOCKET_ERROR == WSAIoctl(
		...
		NULL))
	{
		std::cout << "WSAIoctl 未能获取AcceptEx函数指针。错误代码: " << WSAGetLastError() << std::endl;;
		this->_DeInitialize();
		return false;
	}
	...
	if (SOCKET_ERROR == WSAIoctl(
		...
		NULL))
	{
		std::cout<<"WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: "<<  WSAGetLastError() <<"\n";
		this->_DeInitialize();
		return false;
	}
	...
	//std::cout<<(_T("投递 %d 个AcceptEx请求完毕"), MAX_POST_ACCEPT);
...
bool CIOCPModel::_PostAccept(PER_IO_CONTEXT* pAcceptIoContext)
{
	...
	{
		std::cout << "创建用于Accept的Socket失败！错误代码: " << WSAGetLastError()<<std::endl;
		return false;
	}
...
bool CIOCPModel::_DoAccpet(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext)
{
	...
	std::cout << "客户端 " << inet_ntoa(ClientAddr->sin_addr) << ':'<< ntohs(ClientAddr->sin_port) << "连入. \n";
	std::cout << "客户额 " << inet_ntoa(ClientAddr->sin_addr) <<':'<< ntohs(ClientAddr->sin_port) << " 信息：" << pIoContext->m_wsaBuf.buf << std::endl;
	...
	if (false == this->_AssociateWithIOCP(pNewSocketContext))
	{
		...
		printf("warning : accept will not post again!!!");
		return false;
	}
...
bool CIOCPModel::_AssociateWithIOCP(PER_SOCKET_CONTEXT *pContext)
{
	...
	if (NULL == hTemp)
	{
		std::cout<<"执行CreateIoCompletionPort()出现错误.错误代码："<< GetLastError()<<std::endl;
		return false;
	}
...
{
	std::cout << "occur some error!!!" << dwErr << std::endl;
	// 如果是超时了，就再继续等吧  
